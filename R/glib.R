...
function(interval, f, data = NULL)
{
 ...
 .RGtkCall("R_addGTimeoutHandler", as.integer(interval),  f, data, useData, PACKAGE = "RGtk2.10")
}
...
function(id)
{
  ...
  .Call("R_removeGSource", as.integer(id), PACKAGE = "RGtk2.10")
}
...
function(f, data = NULL)
{
 ...
 if(!is.function(f)) {
  ...
  .RGtkCall("R_addGIdleHandler", f, data, useData, PACKAGE = "RGtk2.10")
}
...
{
	if (is.character(x))
		x <- .Call("R_gQuarkFromString", x, PACKAGE = "RGtk2.10")
	else {
		x <- as.numeric(x)
		class(x) <- "GQuark"
	}
...
function()
{
	...
	w <- .RGtkCall("S_g_file_error_quark", PACKAGE = "RGtk2.10")
