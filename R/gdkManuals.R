...
function (object, gc, x, y, width, height, dith, rgb.buf, rowstride) 
{
    ...
    w <- .RGtkCall("S_gdk_draw_rgb_image", object, gc, x, y, 
        width, height, dith, rgb.buf, rowstride, PACKAGE = "RGtk2.10")
    return(w)
}
...
function(object, region, child.func, user.data)
{
        ...
        w <- .RGtkCall("S_gdk_window_invalidate_maybe_recurse", object, region, child.func, user.data, PACKAGE = "RGtk2.10")
...
function()
{
	...
	w <- .RGtkCall("S_gdk_pixbuf_error_quark", PACKAGE = "RGtk2.10")
